...
const SCKEY = process.env.SCKEY; //SEVER-酱
const BARK_PUSH = process.env.BARK_PUSH; //
const PUSH_KEY = process.env.PUSH_KEY; //
const TG_BOT_TOKEN = process.env.TG_BOT_TOKEN; //TG通知
const TG_USER_ID = process.env.TG_USER_ID; //
 
//非必要
const JXNCTOKENS = process.env.JXNCTOKENS; //西农场种子
...
async function executeOneByOne() {
    ...
    for (var i = 0; i < CookieJDs.length; i++) {
        ...
        $.nickName = '';
        message = ''
       console.log(`\n******开始【🐕号${$.index}】${$.nickName.slice(-4) || $.UserName.slice(-4)}*********\n`);
